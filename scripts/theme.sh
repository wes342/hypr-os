...
# ── Sway/SwayFX theme.conf ───────────────
mkdir -p "$CONFIG_DIR/sway"
cat > "$CONFIG_DIR/sway/theme.conf" << EOF
# Auto-generated by theme.sh from: $(basename "$WALLPAPER")
set \$bg $bg
set \$bg_dim $bg_dim
set \$bg_highlight $bg_highlight
set \$fg $fg
set \$fg_dim $fg_dim
set \$accent $accent
...
client.focused_inactive \$bg_highlight \$bg_highlight \$fg_dim \$bg_highlight \$bg_highlight
...
timestr=%I:%M %p
datestr=%A, %B %d
key-hl-color=$(strip_hash "$accent")
ring-color=$(strip_hash "$accent_dim")
ring-ver-color=$(strip_hash "$accent")
ring-wrong-color=$(strip_hash "$color1")
inside-color=$(strip_hash "$bg")cc
inside-ver-color=$(strip_hash "$bg")cc
inside-wrong-color=$(strip_hash "$bg")cc
text-color=$(strip_hash "$fg")
text-ver-color=$(strip_hash "$fg")
text-wrong-color=$(strip_hash "$fg")
line-color=00000000
...
# hypr-os mako notification config
# Auto-updated by theme.sh
...
if pgrep -x eww >/dev/null 2>&1; then
    ...
    if [[ -x "$HOME/.config/sway/scripts/sensor-panel.sh" ]]; then
        (sleep 1 && "$HOME/.config/sway/scripts/sensor-panel.sh") &
    else
        (sleep 1 && ~/.config/hypr/scripts/sensor-panel.sh) &
    fi
fi

# Update fastfetch terminal image if set to follow wallpaper
SETTINGS_CONF="$HOME/.config/hypr-os/settings.conf"
if [[ -f "$SETTINGS_CONF" ]] && grep -q '^terminal_image=wallpaper' "$SETTINGS_CONF" 2>/dev/null; then
    LOGO_PATH="$(readlink -f "$HOME/.config/fastfetch/logo.png" 2>/dev/null || echo "$HOME/.config/fastfetch/logo.png")"
    magick "$WALLPAPER" -resize "x320" -gravity center -crop "320x320+0+0" +repage -background none "$LOGO_PATH" 2>/dev/null || true
    rm -rf "$HOME/.cache/fastfetch/images" 2>/dev/null || true
fi
